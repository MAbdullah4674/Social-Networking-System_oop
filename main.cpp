...
void showWelcomePage() {
    // Create SFML window
    sf::RenderWindow window(sf::VideoMode(800, 600), "Welcome to Social Network App");

    sf::Texture backgroundImage;
    if (!backgroundImage.loadFromFile("C:/Users/NEW/Downloads/Welcome.jpeg"))//==================================================== 
    {
    ...
    sf::Font font;
    if (!font.loadFromFile("C:/Users/NEW/Downloads/Call of Ops Duty.otf")) {
        // Handle error if font cannot be loaded
        std::cerr << "Failed to load font!" << std::endl;
        return;
    }

    sf::Text welcomeText("Welcome to Social Network App", font, 45);
    welcomeText.setFillColor(sf::Color::Red);
    welcomeText.setPosition(65, 50);
...
public:
	static char *GetStringFromBuffer(char *str)
	{
		...
		strcpy(string1, str); // re-usability of existing functions
...
public:
	...
	void SetValues(char *text, Object *user)
	{
		...
		cout << endl
			 << "new comment added with the ID " << ID << endl;
...
public:
	...
	void AddComment(Comment *commentID)
	{
		...
		if (totalComments < 10) // max count is 10
		{
			...
			cout << endl
				 << GetID() << " got a new comment: " << commentID->GetID() << endl;
		}
		else
...
public:
	...
	void LikePage(Page *ptr)
	{
		if (totalLikedPages < 10) // hardcoded max count 10
		{
			...
			cout << endl
				 << GetName() << " liked the page " << ptr->GetName() << endl;
		}
		else
	...
	void AddFriend(User *ptr)
	{
		if (friendCounter < 10) // max count is 10
		{
			...
			cout << endl
				 << GetName() << " added a new friend: " << ptr->GetName() << endl;
		}
		else
...
int inorup(void) //                Asks for sign in or sign up.
{
	int j;
	cout << "Do you want yo do\tSign Up, then press 2 , else 1 :\t     ";
	cin >> j;
	return j;
}

void dochoice(int ch, const char *pagesfile, const char *usersfile, const char *postsfile, const char *commentsfile)
{
	if (ch == 2)
	{
		string name;
		...
		cin>> name;
		cout << "your user code is :\tu" << ((User::totalUsers) + 1)<<". You should always remember this code.\n\n";
		User::totalUsers += 1;

		ofstream fin(usersfile, ios::app);

		fin << "\nu" << User::totalUsers << "  " << name;
	}
}
...
int main() {
    ...
    cout << "Total Users : " << User::totalUsers << endl
         << "Total Posts : " << Post::totalPosts << endl
         << "Total Pages : " << Page::totalPages << endl
         << "Total Comments : " << Comment::totalComments << endl;
    int choice = inorup();
    dochoice(choice, "Pages.txt", "Users.txt", "Posts.txt", "Comments.txt");
    cout << "Total Users : " << User::totalUsers << endl
         << "Total Posts : " << Post::totalPosts << endl
         << "Total Pages : " << Page::totalPages << endl
         << "Total Comments : " << Comment::totalComments << endl;
